...
def main():
    ...
    for epoch in epoch_iter:
        ...
        if val_metrics["f1"] > best_f1:
            ...
            patience_counter = 0
            model_path = Path(paths["models_dir"]) / "dl_best.pth"
    ...
    log_path = Path(paths["logs_dir"]) / "dl_training_log.csv"
    ...
    metrics_path = Path(paths["outputs_dir"]) / "dl_val_metrics.json"
    ensure_dir(metrics_path.parent)
    with open(metrics_path, "w", encoding="utf-8") as f:
        json.dump({"best_f1": best_f1, "history": history}, f, indent=2)
    print(f"Best F1: {best_f1:.4f}")
    print(f"Saved best model to {Path(paths['models_dir']) / 'dl_best.pth'}")
