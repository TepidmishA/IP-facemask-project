...
def parse_args():
    parser = argparse.ArgumentParser(description="Train classical HOG + LogisticRegression model for mask detection.")
...
def main():
    ...
    paths = config["paths"]
    params = config["training"]["classical"]
    ...
    model_path = models_dir / "classical.joblib"
    save_joblib(
        {
            "model": clf,
            "scaler": scaler,
    ...
    metrics_path = Path(paths["outputs_dir"]) / "classical_val_metrics.json"
